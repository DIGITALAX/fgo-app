...
export const ItemAuthorized = ({ item }: ItemAuthorizedProps) => {
  ...
  const hasAuthorized =
    (item.authorizedChildren && item.authorizedChildren.length > 0) ||
    ((item as Child).authorizedParents &&
      (item as Child).authorizedParents.length > 0) ||
    (item.authorizedTemplates && item.authorizedTemplates.length > 0);
    ...
    <div className="border border-white rounded-sm p-6">
      ...
      {item.authorizedChildren && item.authorizedChildren.length > 0 && (
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {item.authorizedChildren.map((child, index: number) => (
              <button
                key={index}
                onClick={() =>
        ...
        (item as Child).authorizedParents.length > 0 && (
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {(item as Child).authorizedParents.map(
                (parent, index: number) => (
                  <div
                    key={index}
      ...
      {item.authorizedTemplates && item.authorizedTemplates.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {item.authorizedTemplates.map((template, index: number) => (
              <div
                key={index}
                className="bg-black/20 border border-white/30 rounded-sm overflow-hidden"
              >
