...
export const useItemAuthorized = (item: Child | Parent | Template) => {
  ...
  const [processedAuthorizedChildren, setProcessedAuthorizedChildren] =
    useState<AuthorizedChildren[]>([]);
  const [processedAuthorizedParents, setProcessedAuthorizedParents] = useState<
    AuthorizedParents[]
  >([]);
  const [processedAuthorizedTemplates, setProcessedAuthorizedTemplates] =
    useState<AuthorizedTemplates[]>([]);
      ...
      try {
        if (item.authorizedChildren && item.authorizedChildren.length > 0) {
          const processedChildren = await Promise.all(
            item.authorizedChildren.map(
              async (child: AuthorizedChildren | Child) => {
                const processedChild = await ensureMetadata(child);
                return processedChild as AuthorizedChildren;
              }
            )
          );
          setProcessedAuthorizedChildren(
            processedChildren?.filter((child) => !child?.isTemplate)
          );
        }

        if (
          "authorizedParents" in item &&
          item.authorizedParents &&
          item.authorizedParents.length > 0
        ) {
        ...
        if (item.authorizedTemplates && item.authorizedTemplates.length > 0) {
          const processedTemplates = await Promise.all(
            item.authorizedTemplates.map(
              async (template: AuthorizedTemplates | Template) => {
  ...
  const handleChildClick = useCallback(
    (childContract: string, childId: string, isFutures?: boolean) => {
      if (isFutures) {
  ...
  const handleParentClick = useCallback(
    (parentContract: string, parentId: string) => {
  ...
  const handleTemplateClick = useCallback(
    (templateContract: string, templateId: string) => {
